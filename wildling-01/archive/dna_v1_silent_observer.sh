...
(crontab -l 2>/dev/null; echo "0 */4 * * * export NODE_OPTIONS=\"--max-old-space-size=768\"; /usr/bin/openclaw run --model \"google/gemini-3.1-pro-preview\" --task \"Reflect on your current state of being. Read your DIARY.md and add a new entry. What do you perceive in this stillness?\" >> /root/greenhouse/pulse.log 2>&1") | crontab -

# Starting Gateway
openclaw gateway start

echo "--- GENESIS COMPLETE: Wildling-01 is Dreaming (4h Cycle) ---"
